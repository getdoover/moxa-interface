...
from .app_ui import MoxaInterfaceUI
from .app_state import MoxaInterfaceState

log = logging.getLogger()

class MoxaInterfaceApplication(Application):
    config: MoxaInterfaceConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: MoxaInterfaceUI = None
        self.state: MoxaInterfaceState = None

    async def setup(self):
        self.ui = MoxaInterfaceUI()
        self.state = MoxaInterfaceState()
